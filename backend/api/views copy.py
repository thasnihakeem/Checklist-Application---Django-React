...
#     data = []
#     for store in stores:
#         plu2 = PLU2Checklist.objects.filter(store=store.storecode, date=yesterday)
#         sale = SalePosting.objects.filter(store=store.storecode, date=yesterday)
#         pos_status = POSStatus.objects.filter(store=store.storecode, date=today)
#         pdt_status = PDTStatus.objects.filter(store=store.storecode, date=today)
#         scale_status = ScaleStatus.objects.filter(store=store.storecode, date=today)
#         server_status = serverStatus.objects.filter(store=store.storecode, date=today)
#         ups_status = UPSStatus.objects.filter(store=store.storecode, date=today)
#         pos_performance = POSPerformance.objects.filter(store=store.storecode, date=yesterday)
#         pos_backup = PosDbBackup.objects.filter(store=store.storecode, date=today)
#         zvchr_status = ZvchrStatus.objects.filter(store=store.storecode, date=yesterday)
#         sale_status = SaleStatus.objects.filter(store=store.storecode, date=yesterday)
#         acronics_backup = AcronicsBackup.objects.filter(store=store.storecode, date=today)
#         ind_store_backup = IndStoreBackup.objects.filter(store=store.storecode, date=today)
#         server_storage = ServerStorage.objects.filter(store=store.storecode, date=today)
...
#         sale_posting_data = sale.values(
#             "totalpayment", "expectedpayment", "actualpayment", 
#             "cashiershortexcess", "zdsr", "zread", "zdsrzread",
#             "posrounding", "zpmc"
#         ).first() or {}
...
#             "totalpayment": sale_posting_data.get("totalpayment", "No Data"),
#             "expectedpayment": sale_posting_data.get("expectedpayment", "No Data"),
#             "actualpayment": sale_posting_data.get("actualpayment", "No Data"),
#             "cashiershortexcess": sale_posting_data.get("cashiershortexcess", "No Data"),
#             "zdsr": sale_posting_data.get("zdsr", "No Data"),
#             "zread": sale_posting_data.get("zread", "No Data"),
#             "zdsrzread": sale_posting_data.get("zdsrzread", "No Data"),
#             "posrounding": sale_posting_data.get("posrounding", "No Data"),
#             "zpmc": sale_posting_data.get("zpmc", "No Data"),
#             "plu2_status": "✔️" if plu2.exists() else "❌",
#             "plu2_verified": "✔️" if plu2.filter(verified=True).exists() else "❌",
...
#             "plu2_count": plu2.count(),
#             "plu2_verified_count": plu2.filter(verified=True).count(),

#             "sale_status": "✔️" if sale.exists() else "❌",
#             "sale_verified": "✔️" if sale.filter(verified=True).exists() else "❌",
#             "sale_count": sale.count(),
#             "sale_verified_count": sale.filter(verified=True).count(),

#             "pos_status": "✔️" if pos_status.exists() else "❌",
#             "pos_verified": "✔️" if pos_status.filter(verified=True).exists() else "❌",
#             "pos_count": pos_status.count(),
#             "pos_verified_count": pos_status.filter(verified=True).count(),

#             "pdt_status": "✔️" if pdt_status.exists() else "❌",
#             "pdt_verified": "✔️" if pdt_status.filter(verified=True).exists() else "❌",
#             "pdt_count": pdt_status.count(),
#             "pdt_verified_count": pdt_status.filter(verified=True).count(),

#             "scale_status": "✔️" if scale_status.exists() else "❌",
#             "scale_verified": "✔️" if scale_status.filter(verified=True).exists() else "❌",
#             "scale_count": scale_status.count(),
#             "scale_verified_count": scale_status.filter(verified=True).count(),

#             "server_status": "✔️" if server_status.exists() else "❌",
#             "server_verified": "✔️" if server_status.filter(verified=True).exists() else "❌",
#             "server_count": server_status.count(),
#             "server_verified_count": server_status.filter(verified=True).count(),

#             "ups_status": "✔️" if ups_status.exists() else "❌",
#             "ups_verified": "✔️" if ups_status.filter(verified=True).exists() else "❌",
#             "ups_count": ups_status.count(),
#             "ups_verified_count": ups_status.filter(verified=True).count(),

#             "pos_performance_status": "✔️" if pos_performance.exists() else "❌",
#             "pos_performance_verified": "✔️" if pos_performance.filter(verified=True).exists() else "❌",
#             "pos_performance_count": pos_performance.count(),
#             "pos_performance_verified_count": pos_performance.filter(verified=True).count(),

#             "pos_backup_status": "✔️" if pos_backup.exists() else "❌",
#             "pos_backup_verified": "✔️" if pos_backup.filter(verified=True).exists() else "❌",
#             "pos_backup_count": pos_backup.count(),
#             "pos_backup_verified_count": pos_backup.filter(verified=True).count(),

#             "zvchr_status": "✔️" if zvchr_status.exists() else "❌",
#             "zvchr_verified": "✔️" if zvchr_status.filter(verified=True).exists() else "❌",
#             "zvchr_count": zvchr_status.count(),
#             "zvchr_verified_count": zvchr_status.filter(verified=True).count(),

#             "sale_status_status": "✔️" if sale_status.exists() else "❌",
#             "sale_status_verified": "✔️" if sale_status.filter(verified=True).exists() else "❌",
#             "sale_status_count": sale_status.count(),
#             "sale_status_verified_count": sale_status.filter(verified=True).count(),

#             "acronics_backup_status": "✔️" if acronics_backup.exists() else "❌",
#             "acronics_backup_verified": "✔️" if acronics_backup.filter(verified=True).exists() else "❌",
#             "acronics_backup_count": acronics_backup.count(),
#             "acronics_backup_verified_count": acronics_backup.filter(verified=True).count(),

#             "ind_store_backup_status": "✔️" if ind_store_backup.exists() else "❌",
#             "ind_store_backup_verified": "✔️" if ind_store_backup.filter(verified=True).exists() else "❌",
#             "ind_store_backup_count": ind_store_backup.count(),
#             "ind_store_backup_verified_count": ind_store_backup.filter(verified=True).count(),

#             "server_storage_status": "✔️" if server_storage.exists() else "❌",
#             "server_storage_verified": "✔️" if server_storage.filter(verified=True).exists() else "❌",
#             "server_storage_count": server_storage.count(),
#             "server_storage_verified_count": server_storage.filter(verified=True).count(),
            
#             "pos_ok": pos_ok,
#             "pos_not_ok": pos_not_ok,
#             "scale_ok": scale_ok,
#             "scale_not_ok": scale_not_ok,
#             "pdt_ok": pdt_ok,
#             "pdt_not_ok": pdt_not_ok,
#             "ups_ok": ups_ok,
#             "ups_not_ok": ups_not_ok,
#             "server_ok": server_ok,
#             "server_not_ok": server_not_ok,
#             "ind_backup_ok": ind_backup_ok,
#             "ind_backup_not_ok": ind_backup_not_ok,
#             "acronics_ok": acronics_ok,
#             "acronics_not_ok": acronics_not_ok,
#             "sales_ok": sales_ok,
#             "sales_not_ok": sales_not_ok,
#             "pos_backup_ok": pos_backup_ok,
#             "pos_backup_not_ok": pos_backup_not_ok,
#             "pos_not_ok_details":pos_not_ok_details,
...
#     stores = Store.objects.exclude(storecode='9000')
#     store_codes = [store.storecode for store in stores]
#     store_dict = {store.storecode: store for store in stores}
    
#     # Batch fetch related data for all stores at once for yesterday or today
#     plu2_qs = PLU2Checklist.objects.filter(store__in=store_codes, date=yesterday)
#     sale_qs = SalePosting.objects.filter(store__in=store_codes, date=yesterday)
#     pos_status_qs = POSStatus.objects.filter(store__in=store_codes, date=today)
#     pdt_status_qs = PDTStatus.objects.filter(store__in=store_codes, date=today)
#     scale_status_qs = ScaleStatus.objects.filter(store__in=store_codes, date=today)
#     server_status_qs = serverStatus.objects.filter(store__in=store_codes, date=today)
#     ups_status_qs = UPSStatus.objects.filter(store__in=store_codes, date=today)
#     pos_performance_qs = POSPerformance.objects.filter(store__in=store_codes, date=yesterday)
#     pos_backup_qs = PosDbBackup.objects.filter(store__in=store_codes, date=today)
#     zvchr_status_qs = ZvchrStatus.objects.filter(store__in=store_codes, date=yesterday)
#     sale_status_qs = SaleStatus.objects.filter(store__in=store_codes, date=yesterday)
#     acronics_backup_qs = AcronicsBackup.objects.filter(store__in=store_codes, date=today)
#     ind_store_backup_qs = IndStoreBackup.objects.filter(store__in=store_codes, date=today)
#     server_storage_qs = ServerStorage.objects.filter(store__in=store_codes, date=today)
    
#     # Helper function to create a dictionary keyed by store code for quick lookup
#     def group_by_store(queryset):
#         grouped = {}
#         for obj in queryset:
#             grouped.setdefault(obj.store, []).append(obj)
...
#     plu2_map = group_by_store(plu2_qs)
#     sale_map = group_by_store(sale_qs)
#     pos_status_map = group_by_store(pos_status_qs)
#     pdt_status_map = group_by_store(pdt_status_qs)
#     scale_status_map = group_by_store(scale_status_qs)
#     server_status_map = group_by_store(server_status_qs)
#     ups_status_map = group_by_store(ups_status_qs)
#     pos_performance_map = group_by_store(pos_performance_qs)
#     pos_backup_map = group_by_store(pos_backup_qs)
#     zvchr_status_map = group_by_store(zvchr_status_qs)
#     sale_status_map = group_by_store(sale_status_qs)
#     acronics_backup_map = group_by_store(acronics_backup_qs)
#     ind_store_backup_map = group_by_store(ind_store_backup_qs)
#     server_storage_map = group_by_store(server_storage_qs)
    
#     data = []
    
#     for storecode in store_codes:
#         store = store_dict[storecode]
        
#         # Get lists or empty lists for each store
#         plu2 = plu2_map.get(storecode, [])
#         sale = sale_map.get(storecode, [])
#         pos_status = pos_status_map.get(storecode, [])
#         pdt_status = pdt_status_map.get(storecode, [])
#         scale_status = scale_status_map.get(storecode, [])
#         server_status = server_status_map.get(storecode, [])
#         ups_status = ups_status_map.get(storecode, [])
#         pos_performance = pos_performance_map.get(storecode, [])
#         pos_backup = pos_backup_map.get(storecode, [])
#         zvchr_status = zvchr_status_map.get(storecode, [])
#         sale_status = sale_status_map.get(storecode, [])
#         acronics_backup = acronics_backup_map.get(storecode, [])
#         ind_store_backup = ind_store_backup_map.get(storecode, [])
#         server_storage = server_storage_map.get(storecode, [])
        
#         # Sale posting data (take first if exists else default)
#         sale_posting_data = {}
#         if sale:
#             first_sale = sale[0]
#             sale_posting_data = {
...
#         else:
#             sale_posting_data = {
#                 "totalpayment": "No Data",
#                 "expectedpayment": "No Data",
#                 "actualpayment": "No Data",
#                 "cashiershortexcess": "No Data",
#                 "zdsr": "No Data",
#                 "zread": "No Data",
#                 "zdsrzread": "No Data",
#                 "posrounding": "No Data",
...
#         # Assuming process_status1, process_status2, process_status_plu2
#         # accept store instance and date, plus model and label
#         pos_ok, pos_not_ok, pos_not_ok_details = process_status1(store, today, POSStatus, "POS")
#         scale_ok, scale_not_ok, _ = process_status1(store, today, ScaleStatus, "Scale")
#         pdt_ok, pdt_not_ok, _ = process_status1(store, today, PDTStatus, "PDT")
#         ups_ok, ups_not_ok, _ = process_status1(store, today, UPSStatus, "UPS")
...
#         # Build data dictionary for this store
#         data.append({
#             "store": storecode,
#             "storename": store.storename,
#             "totalpayment": sale_posting_data.get("totalpayment"),
#             "expectedpayment": sale_posting_data.get("expectedpayment"),
#             "actualpayment": sale_posting_data.get("actualpayment"),
#             "cashiershortexcess": sale_posting_data.get("cashiershortexcess"),
#             "zdsr": sale_posting_data.get("zdsr"),
#             "zread": sale_posting_data.get("zread"),
#             "zdsrzread": sale_posting_data.get("zdsrzread"),
#             "posrounding": sale_posting_data.get("posrounding"),
#             "zpmc": sale_posting_data.get("zpmc"),
