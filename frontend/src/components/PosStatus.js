...
const POSStatus = () => {
  const [formData, setFormData] = useState({
    ...
    storename: "",
    submittedby:"",
    files: [{ posnumber: "", counternumber: "", section: "", complaint: "", actiontaken: "", status: "", remarks: "" }],
  });
  ...
  const fetchDataAuto = async (storeCode, date, status) => {
    try {
      ...
      const params = {
        ...
        status: status != null ? status : ["ok", "not ok"]
      };
  
      const response = await axios.get(`${API_BASE_URL}/api/posstatus/`, { params });
  ...
  const handleSubmit = async (e) => {
    ...
    const incompleteFiles = formData.files.map((file, index) => {
      ...
      if (!file.counternumber) missingFileFields.push('Counter Number');
    ...
    try {
      const checkResponse = await axios.get(`${API_BASE_URL}/api/posstatus/`, {
      ...
      for (let file of formData.files) {
        ...
        if (existingEntry) {
          response = await axios.put(
            `${API_BASE_URL}/api/update-posstatus/${existingEntry.id}/`,
            { ...existingEntry, ...file, user: username }
          );
        } else {
          response = await axios.post(`${API_BASE_URL}/api/submit-posstatus/`, {
            ...dataToPost,
            files: [file],
          });
        }
  ...
  const fetchDataForEdit = async (id) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/posstatus/${id}/`);
  ...
  const handleDownloadCSV = () => {
    ...
    axios
      .get(`${API_BASE_URL}/api/posstatus/download/`, {
        params,
        responseType: "json",
      })
      .then((response) => {
        ...
        link.setAttribute("download", `POS_Status_${date}.csv`);
  ...
  const convertToCSV = (jsonData) => {
    const header = ["Date", "Store", "Store Name", "Section", "POS Number", "Counter Number", "Complaint", "Action taken",  "Status", "Remarks"];
  ...
  const handleDownloadPDF = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const tableColumns = ["Date", "Store", "Store Name", "Section", "POS Number", "Counter Number", "Complaint", "Action taken", "Status", "Remarks"];
  ...
  const handleSaveClick = async () => {
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/update-posstatus/${editRowId}/`,
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (confirmed) {
      try {
        const response = await axios.delete(`${API_BASE_URL}/api/delete-posstatus/${id}/`, {
          data: { user: username }  
        });
        if (response.status === 200) {
  ...
  useEffect(() => {
    if (posData.length > 0) {
      const files = posData.map((pos) => ({
        ...
        counternumber: pos.counternumber,
        section: pos.section,
        complaint: "",
        actiontaken: "",
        status: "",
      ...
      const emptyFiles = [{ posnumber: "", counternumber: "", section:"", complaint: "", actiontaken: "", status: "", remarks: "" }];
  ...
  const fetchHistory = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/posstatus/`, {
  ...
  const getTabsForUserGroup = () => {
    switch (userGroup) {
      case 'Admin_User':
        return (
          <>
            <Tab value={0}>POS Status Form</Tab>
            <Tab value={1}>POS Status</Tab>
            <Tab value={2}>History</Tab>
          </>
        );
      case 'End_User':
        return <Tab value={0}>POS Status Form</Tab>;
    ...
    try {
      ...
      if (idsToVerify.length > 0) {
        await Promise.all(idsToVerify.map(id =>
          axios.put(`${API_BASE_URL}/api/update-posstatus/${id}/`, { 
                    ...
                    <form onSubmit={handleSubmit}>
                      ...
                      {formData.files.map((file, index) => (
                        <Box key={index} sx={{ marginBottom: 2 }}>
                          ...
                          <Grid item xs={0.2}>
                            ...
                            <Grid item xs={1.6}>
                              <TextField
                                ...
                                type="text"
                                fullWidth
                                value={file.counternumber}
                                onChange={(e) => handleFileChange(index, "counternumber", e.target.value)}
                                sx={{ fontSize: '0.9rem', boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)' }}
                              />
                            </Grid>
                            ...
                            <Grid item xs={1}>
                              <FormControl fullWidth>
                                ...
                                <Select
                                  ...
                                  value={file.status || "Ok"} // Default to "Ok"
                          ...
                          {/* Conditional Fields: Complaint & Action Taken (Only when Not Ok) */}
                          {file.status === "Not Ok" && (
                      ...
                      <TableContainer>
                        ...
                        <TableHead>
                            ...
                            <TableRow>
                              ...
                              <TableCell  colSpan={3} align="center"><strong>{submittedData?.store||''}</strong></TableCell>
                              <TableCell style={{ borderBottom: '1px solid #ccc', padding: '10px 25px', whiteSpace: 'nowrap'}}><strong>Store Name</strong></TableCell>
                              <TableCell colSpan={3} align="center"><strong>{submittedData?.storename ||''}</strong></TableCell>
                            </TableRow>
                          </TableHead>
                          <TableBody>
                              ...
                              submittedData.files.map((file, index) => (
                                <React.Fragment key={index}>
                                  <TableRow>
                                    ...
                                    <TableCell style={{ borderBottom: '1px solid #ccc', padding: '10px 25px', whiteSpace: 'nowrap' }}>Counter Number</TableCell>
                                    <TableCell align="center">{file?.counternumber}</TableCell>
                      ...
                      <TableHead sx={{ position: 'sticky', top: 0, backgroundColor: '#f5f5f5', zIndex: 1000}}>
                        <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                          ...
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '10%' }}>
                            <TableSortLabel active={sortConfig.key === 'counternumber'} direction={sortConfig.key === 'counternumber' ? sortConfig.direction : 'asc'} onClick={() => handleSort('counternumber')}>Counter Number</TableSortLabel>
                          </TableCell>
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '10%' }}>
                        ...
                        return (
                          <TableRow key={item.id} hover
                            ...
                            {editRowId === item.id ? (
                              ...
                              <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '5%' }}>{item.posnumber}</TableCell>
                              <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '5%' }}>{item.counternumber}</TableCell>
