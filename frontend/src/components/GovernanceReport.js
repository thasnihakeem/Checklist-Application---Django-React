...
const GovernanceReport = () => {
  ...
  const handleDelete = async (file) => {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/delete-governance-report/`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({
                file_url: file.file_url, // Ensure correct file path
            }),
        });
        ...
        if (response.ok) {
            ...
            setSnackbarMessage(data.error || "Failed to delete the file.");
            setSnackbarSeverity("error");
            setSnackbarOpen(true);
        }
    } catch (error) {
