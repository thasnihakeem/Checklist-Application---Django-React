...
const ScaleStatus = () => {
  const [formData, setFormData] = useState({
    ...
    storename: "",
    submittedby:"",
    files: [{ scalenumber: "", section: "", complaint: "", actiontaken: "", status: "", remarks: "" }],
  });
  ...
  const fetchDataAuto = async (storeCode, date, status) => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/scalestatus/`, { params });
  ...
  const handleSubmit = async (e) => {
    ...
    const incompleteFiles = formData.files.map((file, index) => {
      ...
      if (!file.scalenumber) missingFileFields.push('Scale Number');
    ...
    try {
      const checkResponse = await axios.get(`${API_BASE_URL}/api/scalestatus/`, {
      ...
      for (let file of formData.files) {
        const existingEntry = existingEntries.find(entry =>
          entry.scalenumber === file.scalenumber &&
        ...
        if (existingEntry && existingEntry.verified) {
          setSnackbar({
            open: true,
            message: `Entry for Scale ${file.scalenumber} is already verified and cannot be modified.`,
            severity: 'error',
          });
          continue; 
        }
        ...
        if (existingEntry) {
          response = await axios.put(
            `${API_BASE_URL}/api/update-scalestatus/${existingEntry.id}/`,
            { ...existingEntry, ...file, user: username}
          );
        } else {
          response = await axios.post(`${API_BASE_URL}/api/submit-scalestatus/`, {
            ...dataToPost,
            files: [file],
          });
        }
  ...
  const fetchDataForEdit = async (id) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/scalestatus/${id}/`);
  ...
  const handleDownloadCSV = () => {
    ...
    axios
      .get(`${API_BASE_URL}/api/scalestatus/download/`, {
        params,
        responseType: "json",
      })
      .then((response) => {
        ...
        link.setAttribute("download", `Scale_Status_${date}.csv`);
  ...
  const convertToCSV = (jsonData) => {
    const header = ["Date", "Store", "Store Name", "Section", "Scale Number", "Complaint", "Action taken",  "Status", "Remarks"];
    ...
    axios
      ...
      .then((response) => {
        ...
        const tableColumns = [
          ...
          "Scale Number", "Complaint", "Action taken", 
          "Status", "Remarks"
        ];
        ...
        doc.save(`Scale_Status_${date}.pdf`);
  ...
  //     format: [400, 230],
  //   });
  //   const tableColumns = ["Date", "Store", "Store Name", "Section", "Scale Number", "Complaint", "Action taken",  "Status", "Remarks"];
  ...
  const handleSaveClick = async () => {
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/update-scalestatus/${editRowId}/`,
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (confirmed) {
      try {
        const response = await axios.delete(`${API_BASE_URL}/api/delete-scalestatus/${id}/`, {
          data: { user: username }  
        });
        if (response.status === 200) {
        ...
        if (formData.store) {
            try {
                ...
                const response = await axios.get(`${API_BASE_URL}/api/pospdtscale-numbers/`, {
                    params: { store: storeParam, type: 'Scale' }  // ✅ Send as string
                });

                console.log("Fetched Scale Data:", response.data);
  ...
  useEffect(() => {
    if (scaleData.length > 0) {
      const files = scaleData.map((scale) => ({
        scalenumber: scale.typenumber,
      ...
      const emptyFiles = [{ scalenumber: "", section:"", complaint: "", actiontaken: "", status: "", remarks: "" }];
  ...
  const fetchHistory = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/scalestatus/`, {
  ...
  const getTabsForUserGroup = () => {
    switch (userGroup) {
      case 'Admin_User':
        return (
          <>
            <Tab value={0}>Scale Status Form</Tab>
            <Tab value={1}>Scale Status</Tab>
            <Tab value={2}>History</Tab>
          </>
        );
      case 'End_User':
        return <Tab value={0}>Scale Status Form</Tab>
    ...
    try {
      ...
      if (idsToVerify.length > 0) {
        await Promise.all(idsToVerify.map(id =>
          axios.put(`${API_BASE_URL}/api/update-scalestatus/${id}/`, { 
                              ...
                              <Grid container spacing={1.5}>
                                ...
                                <Grid item xs={1.8}>
                                  <TextField
                                    ...
                                    type="text"
                                    fullWidth
                                    value={file.scalenumber}
                                    onChange={(e) => handleFileChange(index, "scalenumber", e.target.value)}
                                    sx={{
                                      fontSize: '0.9rem',
                                      boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)',
                                    }}
                                ...
                                {Array.isArray(submittedData?.files) && submittedData.files.length > 0 ? (
                                  submittedData.files.map((file, index) => (
                                    <React.Fragment key={index}>
                                      <TableRow>
                                        <TableCell style={{ borderBottom: '1px solid #ccc', padding: '10px 25px', whiteSpace: 'nowrap' }}>Scale Number</TableCell>
                                        <TableCell align="center">{file?.scalenumber}</TableCell>
                      ...
                      <TableHead sx={{ position: 'sticky', top: 0, backgroundColor: '#f5f5f5', zIndex: 1000}}>
                        <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                          ...
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '20%' }}>
                            ...
                            <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '8%' }}>
                            <TableSortLabel active={sortConfig.key === 'scalenumber'} direction={sortConfig.key === 'scalenumber' ? sortConfig.direction : 'asc'} onClick={() => handleSort('scalenumber')}>Scale Number</TableSortLabel>
                            </TableCell>
                          ...
                          <TableRow key={item.id} hover
                            ...
                            {editRowId === item.id ? (
                              <>
                                ...
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                                  <input type="text" name="scalenumber" value={updatedRow.scalenumber|| ''} onChange={handleInputChange} style={{ width: '100%', height: '40px' }} />
                                </TableCell>
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                              ...
                              <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '5%' }}>{item.scalenumber}</TableCell>
