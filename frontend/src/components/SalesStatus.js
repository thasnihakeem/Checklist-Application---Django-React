...
const SalesStatus = () => {
  ...
  const fetchDataAuto = async (storeCode, date) => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/upload-sale-status/`, { params });
  ...
  const handleSubmit = async (e) => {
    ...
    const requiredFields = ['store', 'date', 'submittedby', 'zdsr', 'zread', 'zcasr', 'zpmc'];
    const missingFields = requiredFields.filter(field => !formData[field]);
    
    if ((formData.zdsr === "Not Updated" || formData.zdsr === "Error" ||
      formData.zcasr === "Not Updated" || formData.zcasr === "Error" ||
      formData.zread === "Not Updated" || formData.zread === "Error" ||
      formData.zpmc === "Not Updated" || formData.zpmc === "Error") 
      && !formData.remarks) {
   missingFields.push("remarks");
    ...
    try {
      const checkResponse = await axios.get(`${API_BASE_URL}/api/upload-sale-status/`, {
      ...
      if (checkResponse.data.length > 0) {
        ...
        response = await axios.put(`${API_BASE_URL}/api/upload-sale-status/${existingEntry.id}/`, dataToSubmit);
      } else {
        response = await axios.post(`${API_BASE_URL}/api/upload-sale-status/`, dataToSubmit);
      }
  ...
  const fetchDataForEdit = async (id) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/update-sale-status/${id}/`, {
      ...
      setSnackbarMessage("Failed to fetch data for edit.");
      setSnackbarSeverity("error");
      setSnackbarOpen(true);
    }
  };
  ...
  const handleDownloadCSV = () => {
    ...
    axios
      .get(`${API_BASE_URL}/api/sale-status/download/`, {
        params,
        responseType: 'json',
      })
      .then((response) => {
        ...
        link.setAttribute('download', `Sale_Status_${date}.csv`);
  ...
  function convertToCSV(jsonData) {
    const header = [
      'Date',  'Store', 'Store Name', 'ZDSR', 'ZCASR', 'ZREAD', 'ZPMC', 'Remarks'
    ];
  ...
  const handleDownloadPDF = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const tableColumns = ['Date', 'Store', 'Store Name', 'ZDSR', 'ZCASR', 'ZREAD', 'ZPMC', 'Remarks'];
        ...
        doc.save(`Sale_Status_${date}.pdf`);
  ...
  const handleSaveClick = async () => {
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/sale-status/${editRowId}/`,  
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (confirmed) {
      try {
        const response = await axios.delete(`${API_BASE_URL}/api/sale-status/${id}/`, {
          data: { user: username }  
        });
        if (response.status === 200) {
  ...
  const fetchHistory = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/sale-status/`, {
    ...
    try {
      ...
      if (idsToVerify.length > 0) {
        await Promise.all(idsToVerify.map(id =>
          axios.put(`${API_BASE_URL}/api/upload-sale-status/${id}/`, { 
  ...
  const getTabsForUserGroup = () => {
    switch (userGroup) {
      case 'Admin_User':
        return (
          <>
            <Tab value={0}>Sales Status Form</Tab>
            <Tab value={1}>Sales Status </Tab>
            <Tab value={2}>History</Tab>
          </>
        );
      case 'End_User':
        return <Tab value={0}>Sales Status Form</Tab>;
    ...
    switch (userGroup) {
      case 'Admin_User':
        return (
          <>
            <TabPanel value={0}>
              <Box sx={{ padding: 4, maxWidth: 600, margin: '0 auto', minHeight: '1000px' }}> {/* Increased minHeight */}
                    ...
                    {isEditMode ? (
                      ...
                      <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} md={6}>
                          <Box sx={{ marginBottom: 1.8, boxShadow: 2, borderRadius: 1 }}>
                            <FormControl fullWidth>
                              <InputLabel id="zdsr-label" sx={{ fontSize: '1rem', backgroundColor: 'white', px: 0.5,
                                transform: 'translate(14px, 14px) scale(1)', '&.Mui-focused, &.MuiInputLabel-shrink': {
                                transform: 'translate(14px, -6px) scale(0.75)',},}}>ZDSR</InputLabel>
                              <Select
                                labelId="zdsr-label"
                                name="zdsr"
                                value={formData.zdsr}
                        ...
                        <Grid item xs={12} md={6}>
                          <Box sx={{ marginBottom: 1.8, boxShadow: 2, borderRadius: 1 }}>
                            <FormControl fullWidth>
                              <InputLabel id="zcasr-label" sx={{ fontSize: '1rem', backgroundColor: 'white', px: 0.5,
                                transform: 'translate(14px, 14px) scale(1)', '&.Mui-focused, &.MuiInputLabel-shrink': {
                                transform: 'translate(14px, -6px) scale(0.75)',},}}>ZCASR</InputLabel>
                              <Select
                                labelId="zcasr-label"
                                name="zcasr"
                                value={formData.zcasr}
                                onChange={handleChange}
                                required
                                sx={{
                                  fontSize: '0.8rem',
                        ...
                        <Grid item xs={12} md={6}>
                          <Box sx={{ marginBottom: 1.8, boxShadow: 2, borderRadius: 1 }}>
                            <FormControl fullWidth>
                              <InputLabel id="zread-label" sx={{ fontSize: '1rem', backgroundColor: 'white', px: 0.5,
                                transform: 'translate(14px, 14px) scale(1)', '&.Mui-focused, &.MuiInputLabel-shrink': {
                                transform: 'translate(14px, -6px) scale(0.75)',},}}>ZREAD</InputLabel>
                              <Select
                                labelId="zread-label"
                                name="zread"
                        ...
                        <Grid item xs={12} md={6}>
                          <Box sx={{ marginBottom: 1.8, boxShadow: 2, borderRadius: 1 }}>
                            <FormControl fullWidth>
                              <InputLabel id="zpmc-label" sx={{ fontSize: '1rem', backgroundColor: 'white', px: 0.5,
                                transform: 'translate(14px, 14px) scale(1)', '&.Mui-focused, &.MuiInputLabel-shrink': {
                                transform: 'translate(14px, -6px) scale(0.75)',},}}>ZPMC</InputLabel>
                              <Select
                                labelId="zpmc-label"
                                name="zpmc"
                                value={formData.zpmc}
                          ...
                          <TableContainer>
                            <Table>
                              ...
                              <TableBody>
                                ...
                                <TableRow>
                                  <TableCell>ZDSR</TableCell>
                                  <TableCell align="center">{submittedData?.zdsr}</TableCell>
                                </TableRow>
                                <TableRow>
                                  <TableCell>ZCASR</TableCell>
                                  <TableCell align="center">{submittedData?.zcasr}</TableCell>
                                </TableRow>
                                <TableRow>
                    ...
                    <Table sx={{ borderCollapse: 'collapse', border: '2px solid #113f6c' }}>
                      <TableHead sx={{ position: 'sticky', top: 0, backgroundColor: '#f5f5f5', zIndex: 1000}}>
                        <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                          ...
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color : '#113f6c', fontSize: '0.9rem', width: '10%' }}>
                            <TableSortLabel active={sortConfig.key === 'zcasr'} direction={sortConfig.key === 'zcasr' ? sortConfig.direction : 'asc'} onClick={() => handleSort('zcasr')}>ZCASR</TableSortLabel>
                          </TableCell>
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color : '#113f6c', fontSize: '0.9rem', width: '10%' }}>
                          ...
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color : '#113f6c', fontSize: '0.9rem', width: '10%' }}>
                            <TableSortLabel active={sortConfig.key === 'zpmc'} direction={sortConfig.key === 'zpmc' ? sortConfig.direction : 'asc'} onClick={() => handleSort('zpmc')}>ZPMC</TableSortLabel>
                          </TableCell>
                            ...
                            {editRowId === item.id ? (
                              <>
                                ...
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                                  <Select  name="zdsr"  value={updatedRow.zdsr || ''}  onChange={handleInputChange}  fullWidth 
                                ...
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                                  <Select  name="zcasr"   value={updatedRow.zcasr|| ''}  onChange={handleInputChange} fullWidth
                                    sx={{ fontSize: '0.8rem', height: '40px', '& .MuiInputBase-input': { textAlign: 'left' } }}
                                    MenuProps={{PaperProps: {sx: {'& .MuiMenuItem-root': {fontSize: '0.8rem', padding: '4px 8px',},},},}}>
                                    <MenuItem value="Updated">Updated</MenuItem>
                                    <MenuItem value="Not Updated">Not Updated</MenuItem>
                                ...
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                                  <Select name="zread" value={updatedRow.zread || ''} onChange={handleInputChange} fullWidth
                                    sx={{ fontSize: '0.8rem', height: '40px', '& .MuiInputBase-input': { textAlign: 'left' } }}
                                    MenuProps={{PaperProps: {sx: {'& .MuiMenuItem-root': { fontSize: '0.8rem', padding: '4px 8px',}, },},}}>
                                    <MenuItem value="Updated">Updated</MenuItem>
                                    <MenuItem value="Not Updated">Not Updated</MenuItem>
                                ...
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                                  <Select name="zpmc" value={updatedRow.zpmc || ''} onChange={handleInputChange} fullWidth
                                    sx={{ fontSize: '0.8rem', height: '40px', '& .MuiInputBase-input': { textAlign: 'left' } }}
                                    MenuProps={{ PaperProps: { sx: {'& .MuiMenuItem-root': {fontSize: '0.8rem', padding: '4px 8px',}, },},}}>
                                    <MenuItem value="Updated">Updated</MenuItem>
                                    <MenuItem value="Not Updated">Not Updated</MenuItem>
                              ...
                              <>
                                ...
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%', color: item.zdsr === 'Not Updated' || item.zdsr === 'Error' ? 'red' : 'black' }}>{item.zdsr}</TableCell>
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%', color: item.zcasr === 'Not Updated' || item.zcasr === 'Error' ? 'red' : 'black', }}>{item.zcasr}</TableCell>
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%', color: item.zread === 'Not Updated' || item.zread === 'Error' ? 'red' : 'black', }}>{item.zread}</TableCell>
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%', color: item.zpmc === 'Not Updated' || item.zpmc === 'Error' ? 'red' : 'black', }}>{item.zpmc}</TableCell>
              ...
              <div>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, marginBottom: 1, marginLeft: 19, marginRight: 10 , marginTop:5 }}>
                  ...
                  <FormControl sx={{ flex: 1 }}>
                    ...
                    <Select
                      labelId="store-select-label"
                      value={store || (profile?.designation === 'userGroup' || profile?.designation === 'IT Manager' ? "None" : "")}
                      onChange={(e) => {
                        setStore(e.target.value);
                        handleStoreChange(e);
                      }}
