...
const Profile = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const requiredFields = ['action', 'storename', 'employeeid', 'designation'];
    ...
    const dataToPost = {
      ...
      storeunder: ["Regional IT Manager", "IT Support", "IT Incharge", "Assistant IT Incharge"].includes(formData.designation)
      ? (typeof formData.storeunder === "string" && formData.storeunder.trim() !== "" 
        ? formData.storeunder 
        : null)
      : null,
    ...
    try {
      const checkResponse = await axios.get(`${API_BASE_URL}/api/manage-profile/`, {
      ...
      if (formData.action === "Add") {
        const existingEntry = existingEntries.find(
          (entry) => entry.employeeid === formData.employeeid
        );
  
        if (existingEntry) {
          response = await axios.put(
            `${API_BASE_URL}/api/update-profile/${existingEntry.id}/`,
            dataToPost
          );
          setSnackbar({
            open: true,
          ...
          response = await axios.post(`${API_BASE_URL}/api/submit-profile/`, dataToPost);
          setSnackbar({
            open: true,
        ...
        if (existingEntry) {
          response = await axios.delete(
            `${API_BASE_URL}/api/delete-profile/${existingEntry.id}/`
          );
  ...
  const fetchDataForEdit = async (id) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/manage-profile/${id}/`);
  ...
  const fetchData = async () => {
    try {
        const params = { store };
        const response = await axios.get(`${API_BASE_URL}/api/fetch-profile/`, { params });
  ...
  const handleSaveClick = async () => {
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/update-profile/${editRowId}/`,
        updatedRow
      );
      if (response.status === 200) {
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (confirmed) {
      try {
        const response = await axios.delete(`${API_BASE_URL}/api/delete-profile/${id}/`);
        if (response.status === 200) {
  ...
  return (
    <div style={{ display: 'flex' }}>
      <Tabs defaultValue={0}>
        <TabsList>
          <Tab value={0}>Manage Profile</Tab>
          <Tab value={1}>Profile Details</Tab>
        </TabsList>
        <TabPanel value={0}>
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <Box sx={{ display: 'flex', gap: 2, marginBottom: 2 }}>
                    <FormControl fullWidth>
                      ...
                      <Select labelId="designation" id="designation" name="designation" value={formData.designation} onChange={handleChange}
                        sx={{ fontSize: '0.8rem', '& .MuiInputBase-input': {  textAlign: 'left', }, }} >
                        <MenuItem value="IT Support">IT Support</MenuItem>
                        <MenuItem value="Assistant IT Incharge">Assistant IT Incharge</MenuItem>
                        <MenuItem value="IT Incharge">IT Incharge</MenuItem>
                        <MenuItem value="Regional IT Manager">Regional IT Manager</MenuItem>
                        <MenuItem value="IT Manager">IT Manager</MenuItem>
                        <MenuItem value="Admin">Admin</MenuItem>
                      </Select>
                    </FormControl>
                  </Box>
                  {['Regional IT Manager', 'IT Support', 'IT Incharge','Assistant IT Incharge' ].includes(formData.designation) && (
                  <Box sx={{ display: 'flex', gap: 2, marginBottom: 2 }}>
                    <TextField
                      label="Stores"
                      fullWidth
                      value={formData.storeunder || ""}
                      onChange={(e) => setFormData({ ...formData, storeunder: e.target.value })}
                      placeholder="Enter store codes separated by commas"
                    />
                  </Box>
                )}
              </form>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          ...
                          <TableRow>
                            ...
                            <TableCell align="center">{formData?.employeeid}</TableCell>
                          </TableRow>
                          <TableRow>
                            <TableCell>Designation</TableCell>
                            <TableCell align="center">{formData?.designation}</TableCell>
                          </TableRow>
                          {["Regional IT Manager", "IT Support"].includes(formData?.designation) && (
                            <>
                              <TableRow>
                                <TableCell style={{ borderBottom: '1px solid #ccc', padding: '10px 25px', whiteSpace: 'nowrap' }}>Stores Under</TableCell>
                                <TableCell colSpan={3} align="center">{formData?.storeunder}</TableCell>
                              </TableRow>
                            </>
                          )}
                        </TableBody>
              ...
              <div style={{ maxHeight: '589px', overflowY: 'auto' , border: '1px solid #113f6c'}}>
                ...
                <TableHead sx={{ position: 'sticky', top: 0, backgroundColor: '#f5f5f5', zIndex: 1000}}>
                    <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                      ...
                      <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '18%' }}>Employee ID</TableCell>
                      <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '18%' }}>Designation</TableCell>
                      <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '18%' }}>Stores Under Regional IT Manager</TableCell>
                      <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '3%' }}></TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                        ...
                        {editRowId === item.id ? (
                          <>
                            ...
                            <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '18%' }}>
                              <input type="text" name="employeeid" value={updatedRow.employeeid || ''} onChange={handleInputChange} style={{ width: '100%', height: '40px' }} />
                            </TableCell>
                            <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                              <Select  name="designation"  value={updatedRow.designation || ''}  onChange={handleInputChange}  fullWidth 
                            ...
                            <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '18%' }}>
                              <input type="text" name="storeunder" value={updatedRow.storeunder || ''} onChange={handleInputChange} style={{ width: '100%', height: '40px' }} />
                            </TableCell>
                          ...
                          <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '20%' }}>{item.employeeid}</TableCell>
                          <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '18%' }}>{item.designation}</TableCell>
                          <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '18%' }}>{item.storeunder}</TableCell>
                          <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.7rem' }}>
                            <Box sx={{ display: 'flex', gap: 1 }}>
