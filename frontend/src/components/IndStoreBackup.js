...
const IndStoreBackupStatus = () => {
  const [formData, setFormData] = useState({
    ...
    remarks: "",
    submittedby:"",
    files: [{ servername: "", tpcentraldb : "", tpcmdb: "" }], 
  });
  ...
  const handleAddFile = () => {
    setFormData({
      ...formData,
      files: [...formData.files, { servername: "", tpcentraldb: "", tpcmdb: "" }],
    });
  };
  ...
  const fetchDataAuto = async (storeCode, date, status) => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/indstorebackup/`, { params });
  
      let filtered;
  
      if (status == null) {
        filtered = response.data.filter(item =>
          item.tpcentraldb?.toLowerCase() === "ok" ||
          item.tpcmdb?.toLowerCase() === "ok" ||
          item.tpcentraldb?.toLowerCase() !== "ok" ||
          item.tpcmdb?.toLowerCase() !== "ok"
        );
      } else {
        filtered = response.data.filter(item =>
          item.tpcentraldb?.toLowerCase() !== "ok" || item.tpcmdb?.toLowerCase() !== "ok"
        );
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const incompleteFiles = formData.files.map((file, index) => {
      ...
      if (!file.tpcentraldb) missingFileFields.push('TP Central DB');
      if (!file.tpcmdb) missingFileFields.push('TPCMDB');
    
      if ((file.tpcentraldb  === "Not Ok" || file.tpcmdb === "Not Ok"  ) && !formData.remarks) {
        missingFileFields.push('Remarks (Required when TP Central DB or TPCMDB is missing)');
      }
    ...
    // If there are incomplete files, display which file has which missing fields
    ...
    try {
      const checkResponse = await axios.get(`${API_BASE_URL}/api/indstorebackup/`, {
      ...
      for (let file of formData.files) {
        ...
        if (existingEntry) {
          response = await axios.put(
            `${API_BASE_URL}/api/update-indstorebackup/${existingEntry.id}/`,
            { ...existingEntry, ...file,  user: username, remarks: formData.remarks }
          );
        } else {
          response = await axios.post(`${API_BASE_URL}/api/submit-indstorebackup/`, {
            ...dataToPost,
            files: [file],
          });
        }
    ...
    try {
      ...
      if (idsToVerify.length > 0) {
        await Promise.all(idsToVerify.map(id =>
          axios.put(`${API_BASE_URL}/api/update-indstorebackup/${id}/`, { 
  ...
  const fetchDataForEdit = async (id) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/indstorebackup/${id}/`);
  ...
  const handleDownloadCSV = () => {
    ...
    axios
      .get(`${API_BASE_URL}/api/indstorebackup/download/`, {
        params,
        responseType: "json",
      })
      .then((response) => {
        ...
        link.setAttribute("download", `Ind_Store_Backup_${date}.csv`);
  ...
  const convertToCSV = (jsonData) => {
    const header = ["Date", "Store", "Store Name",  "Server Name", "TPCentralDB", "TPCMDB", "Remarks"];
  ...
  const handleDownloadPDF = () => {
    ...
    if (store) params.store = store;
    axios.get(`${API_BASE_URL}/api/indstorebackup/download/`, {
        params,
        responseType: "json",
      })
      .then((response) => {
        ...
        const tableColumns = ["Date", "Store", "Store Name", "Server Name", "TPCentralDB", "TPCMDB", "Remarks"];
        ...
        doc.text("Ind Store Backup Data", 14, 10);
        doc.autoTable({
          head: [tableColumns],
          body: tableData,
          startY: 20,
        ...
        doc.save(`Ind_Store_Backup_${date}.pdf`);
  ...
  const handleSaveClick = async () => {
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/update-indstorebackup/${editRowId}/`,
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (confirmed) {
      try {
        const response = await axios.delete(`${API_BASE_URL}/api/delete-indstorebackup/${id}/`, {
          data: { user: username }  
        });
        if (response.status === 200) {
  ...
  const fetchHistory = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/indstorebackup/`, {
  ...
  const getTabsForUserGroup = () => {
    switch (userGroup) {
      case 'Admin_User':
        return (
          <>
            <Tab value={0}>Ind-Store Backup Form</Tab>
            <Tab value={1}>Ind-Store Backup Status</Tab>
            <Tab value={2}>History</Tab>
          </>
        );
      case 'End_User':
        return <Tab value={0}>Ind-Store Backup Form</Tab>;
                      ...
                      {formData.files.map((file, index) => (
                        <Box key={index} sx={{ display: 'flex', gap: 2, marginBottom: 2 }}>
                          <TextField
                            ...
                            type="text"
                            fullWidth
                            value={file.servername}
                            onChange={(e) => handleFileChange(index, "servername", e.target.value)}
                            sx={{
                              fontSize: '0.9rem',
                              boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)',
                            }}
                          ...
                          <FormControl fullWidth>
                            <InputLabel id={`tpcentraldb-label-${index}`} sx={{
                              fontSize: '1rem',
                              backgroundColor: 'white',
                              px: 0.5,
                              transform: 'translate(14px, 14px) scale(1)',
                            ...
                            <Select
                              labelId={`tpcentraldb-label-${index}`}
                              value={file.tpcentraldb}
                              onChange={(e) => handleFileChange(index, "tpcentraldb", e.target.value)}
                              sx={{
                                fontSize: '0.8rem',
                                height: '56px',
                                '& .MuiInputBase-input': {
                          ...
                          <FormControl fullWidth>
                            <InputLabel id={`tpcmdb-label-${index}`} sx={{
                              fontSize: '1rem',
                              backgroundColor: 'white',
                              px: 0.5,
                              transform: 'translate(14px, 14px) scale(1)',
                            ...
                            <Select
                              labelId={`tpcmdb-label-${index}`}
                              value={file.tpcmdb}
                              onChange={(e) => handleFileChange(index, "tpcmdb", e.target.value)}
                              sx={{
                                fontSize: '0.8rem',
                                height: '56px',
                                '& .MuiInputBase-input': {
                              ...
                              submittedData.files.map((file, index) => (
                                <React.Fragment key={index}>
                                  <TableRow>
                                    ...
                                    <TableCell align="center">{file?.servername}</TableCell>
                                    <TableCell style={{ borderBottom: '1px solid #ccc', padding: '10px 25px', whiteSpace: 'nowrap' }}>TPCentralDB</TableCell>
                                    <TableCell align="center">{file?.tpcentraldb}</TableCell>
                                    <TableCell style={{ borderBottom: '1px solid #ccc', padding: '10px 25px', whiteSpace: 'nowrap' }}>TPCMDB</TableCell>
                                    <TableCell align="center">{file?.tpcmdb}</TableCell>
                                  </TableRow>
                                </React.Fragment>
                              ))
                            ) : (
                    ...
                    <Table sx={{ borderCollapse: 'collapse', border: '2px solid #113f6c' }}>
                      <TableHead sx={{ position: 'sticky', top: 0, backgroundColor: '#f5f5f5', zIndex: 1000}}>
                        <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                          ...
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '8%' }}>
                            <TableSortLabel active={sortConfig.key === 'tpcentraldb'} direction={sortConfig.key === 'tpcentraldb' ? sortConfig.direction : 'asc'} onClick={() => handleSort('tpcentraldb')}>TPCentralDB</TableSortLabel>
                          </TableCell>
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '10%' }}>
                            <TableSortLabel active={sortConfig.key === 'tpcmdb'} direction={sortConfig.key === 'tpcmdb' ? sortConfig.direction : 'asc'} onClick={() => handleSort('tpcmdb')}>TPCMDB</TableSortLabel>
                          </TableCell>
                            ...
                            {editRowId === item.id ? (
                              <>
                               <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '5%' }}>
                                ...
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                                  <Select  name="tpcentraldb"  value={updatedRow.tpcentraldb || ''}  onChange={handleInputChange}  fullWidth 
                                ...
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                                  <Select  name="tpcmdb"  value={updatedRow.tpcmdb || ''}  onChange={handleInputChange}  fullWidth 
                                    sx={{ fontSize: '0.8rem', height: '40px', '& .MuiInputBase-input': { textAlign: 'left' } }} 
                                    MenuProps={{ PaperProps: { sx: { '& .MuiMenuItem-root': { fontSize: '0.8rem',  padding: '4px 8px', }, }, }, }} >
                                    <MenuItem value="Ok">Ok</MenuItem>
                                    <MenuItem value="Not Ok">Not Ok</MenuItem>
                              ...
                              <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%', color: item.tpcentraldb === 'Not Ok' ? 'red' : 'black'}}>{item.tpcentraldb}</TableCell>
                              <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%', color: item.tpcmdb === 'Not Ok' ? 'red' : 'black'}}>{item.tpcmdb}</TableCell>
...
export default IndStoreBackupStatus