...
const ServerStatus = () => {
  const [formData, setFormData] = useState({
    ...
    ac:"",
    submittedby:"",
    files: [{  servername: "", serialnumber : "", modelname:"", warrantyexp:"",  amc:"" , status: "", remarks: ""}],
  });
  ...
  const fetchDataAuto = async (storeCode, date, status) => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/serverstatus/`, { params });
  ...
  const handleSubmit = async (e) => {
    ...
    // Check for incomplete files and track which fields are missing
    ...
    try {
      const checkResponse = await axios.get(`${API_BASE_URL}/api/serverstatus/`, {
      ...
      for (let file of formData.files) {
        ...
        if (existingEntry) {
          response = await axios.put(
            `${API_BASE_URL}/api/update-serverstatus/${existingEntry.id}/`,
            { ...existingEntry, ...file, ac: formData.ac, user: username, }
          );
        } else {
          response = await axios.post(`${API_BASE_URL}/api/submit-serverstatus/`, {
            ...dataToPost,
            files: [file],
          });
        }
  ...
  const fetchDataForEdit = async (id) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/serverstatus/${id}/`);
  ...
  const handleDownloadCSV = () => {
    ...
    axios
      .get(`${API_BASE_URL}/api/serverstatus/download/`, {
        params,
        responseType: "json",
      })
      .then((response) => {
        ...
        link.setAttribute("download", `Server_Status_${date}.csv`);
  ...
  const convertToCSV = (jsonData) => {
    const header = ["Date", "Store", "Store Name", "Server Name", "Serial Number", "Model Name", "Warranty Date", "AMC","AC", "Status", "Remarks"];
  ...
  const handleDownloadPDF = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const tableColumns = [
          "Date", "Store", "Store Name", "Store", "Server Name",
          "Serial Number", "Model Name", "Warranty Date", "AMC", "AC", "Status", "Remarks"
        ];
        ...
        const tableData = jsonData.map((item) => [
          ...
          item.store,
          item.storename,
          item.store, // repeated as per original code
          item.servername,
          item.serialnumber,
          item.modelname,
          item.warrantyexp,
        ...
        doc.save(`Server_Status_${date}.pdf`);
  ...
  //     format: [400, 230],
  //   });
  //   const tableColumns = ["Date", "Store", "Store Name", "Store", "Server Name", "Serial Number", "Model Name", "Warranty Date", "AMC","AC", "Status", "Remarks"];
  ...
  const handleSaveClick = async () => {
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/update-serverstatus/${editRowId}/`,
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (confirmed) {
      try {
        const response = await axios.delete(`${API_BASE_URL}/api/delete-serverstatus/${id}/`, {
          data: { user: username }  
        });
...
// Fetch server details when formData.store changes
useEffect(() => {
        ...
        if (formData.store) {
            try {
              ...
              : formData.store;
                ...
                const response = await axios.get(`${API_BASE_URL}/api/serverlist/`, {
                ...
                setFormData(prev => ({
                    ...prev,
                    serverDetails: Array.isArray(response.data) ? response.data : [response.data],
                }));
            } catch (error) {
                console.error("Error fetching server details:", error);
                setFormData(prev => ({ ...prev, serverDetails: [] }));
            }
        }
    };
...
useEffect(() => {
  ...
  useEffect(() => {
    if (formData.serverDetails && Array.isArray(formData.serverDetails)) {
      const files = formData.serverDetails.map((server) => ({
        servername: server.servername || "",
        serialnumber: server.serialnumber || "",
        modelname: server.modelname || "",
        warrantyexp: server.warrantyexp || "",
        amc: server.amc || "",
        status: "",
        remarks: "",
      }));
  ...
  }, [formData.serverDetails]);
  
  const handleFileChange = (index, field, value) => {
    const updatedFiles = [...formData.files];  // Copy the files array
    updatedFiles[index] = { 
      ...updatedFiles[index], // Copy the specific file object
  ...
  const handleAddFile = () => {
    setFormData({
      ...
      files: [
        ...formData.files,
        {
          servername: "", serialnumber : "", modelname:"", warrantyexp:"", amc:"" ,
          status: "",
          remarks: "",
        },
      ],
  ...
  const fetchHistory = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/serverstatus/`, {
    ...
    try {
      ...
      if (idsToVerify.length > 0) {
        await Promise.all(idsToVerify.map(id =>
          axios.put(`${API_BASE_URL}/api/update-serverstatus/${id}/`, { 
  ...
  const getTabsForUserGroup = () => {
    switch (userGroup) {
      case 'Admin_User':
        return (
          <>
            <Tab value={0}>Server Status Form</Tab>
            <Tab value={1}>Server Status</Tab>
            <Tab value={2}>History</Tab>
          </>
        );
      case 'End_User':
        return <Tab value={0}>Server Status Form</Tab>
    ...
    switch (userGroup) {
      case 'Admin_User':
        return (
          <>
            <TabPanel value={0}>
              <Box sx={{ padding: 4, maxWidth: 1350, margin: '0 auto', minHeight: '1000px', marginTop:"-20px"}}>
                          ...
                          <Grid container spacing={1.5} alignItems="center">
                            ...
                            <Grid item xs={3.9}>
                              <TextField
                                ...
                                onChange={(e) => handleFileChange(index, "modelname", e.target.value)}
                                sx={{
                                  fontSize: '0.9rem',
                                  boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)',
                                }}
                            ...
                            <Grid item xs={3}>
                              <TextField
                                ...
                                rows={1}
                                value={file.amc}
                                onChange={(e) => handleFileChange(index, "amc", e.target.value)}
                                sx={{
                                  boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)',
                                }}
                              />
                        ...
                        return (
                          <TableRow key={item.id} hover
                            ...
                            {editRowId === item.id ? (
                              ...
                              <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '20%'}}>{item.ac}</TableCell>
                              <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '8%' }}>{item.remarks}</TableCell>
                          ...
                          <Grid container spacing={1.5} alignItems="center">
                            ...
                            <Grid item xs={4}>
                              <TextField
                                ...
                                fullWidth
                                value={file.servername}
                                onChange={(e) => handleStoreChange(index, "servername", e.target.value)}
                                sx={{
                                  fontSize: '0.9rem',
                                  boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)',
                                }}
