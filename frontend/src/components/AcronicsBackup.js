...
const AcronicsBackupStatus = () => {
  ...
  const fetchDataAuto = async (storeCode, date, status) => {
    try {
      ...
      const params = {
        ...
        status: status || null // pass both if not explicitly provided
      };
  
      const response = await axios.get(`${API_BASE_URL}/api/acronicsbackup/`, { params });
  ...
  const handleSubmit = async (e) => {
    ...
    const incompleteFiles = formData.files.map((file, index) => {
      ...
      if (!file.tpcentraldb) missingFileFields.push('TPCentralDB');
    ...
    try {
      const checkResponse = await axios.get(`${API_BASE_URL}/api/acronicsbackup/`, {
      ...
      for (let file of formData.files) {
        ...
        if (existingEntry) {
          response = await axios.put(
            `${API_BASE_URL}/api/update-acronicsbackup/${existingEntry.id}/`,
            { ...existingEntry, ...file, user: username, remarks: formData.remarks}
          );
          savedEntries.push(response.data);
        } else {
          response = await axios.post(`${API_BASE_URL}/api/submit-acronicsbackup/`, {
            ...dataToPost,
            files: [file],
          });
          savedEntries.push(response.data);
    ...
    try {
      ...
      if (idsToVerify.length > 0) {
        await Promise.all(idsToVerify.map(id =>
          axios.put(`${API_BASE_URL}/api/update-acronicsbackup/${id}/`, { 
  ...
  const fetchDataForEdit = async (id) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/acronicsbackup/${id}/`);
  ...
  const handleDownloadCSV = () => {
    ...
    axios
      .get(`${API_BASE_URL}/api/acronicsbackup/download/`, {
        params,
        responseType: "json",
      })
      .then((response) => {
        ...
        link.setAttribute("download", `Acronics_Backup_${date}.csv`);
  ...
  const handleDownloadPDF = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        doc.text("Acronics Backup Data", 14, 10);
        doc.autoTable({
          head: [tableColumns],
          body: tableData,
          startY: 20,
        ...
        doc.save(`Acronics_Backup_${date}.pdf`);
  ...
  const handleSaveClick = async () => {
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/update-acronicsbackup/${editRowId}/`,
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (confirmed) {
      try {
        const response = await axios.delete(`${API_BASE_URL}/api/delete-acronicsbackup/${id}/`, {
          data: { user: username }  
        });
        if (response.status === 200) {
  ...
  const fetchHistory = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/acronicsbackup/`, {
  ...
  const getTabsForUserGroup = () => {
    switch (userGroup) {
      case 'Admin_User':
        return (
          <>
            <Tab value={0}>Acronics Backup Form</Tab>
            <Tab value={1}>Acronics Backup Status</Tab>
            <Tab value={2}>History</Tab>
          </>
        );
      case 'End_User':
        return <Tab value={0}>Acronics Backup Form</Tab>;
...
export default AcronicsBackupStatus