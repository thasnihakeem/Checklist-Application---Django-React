...
const ServerStorageStatus = () => {
  const [formData, setFormData] = useState({
    ...
    remarks: "",
    submittedby:"",
    files: [{ servername: "", harddrive : "", totalspace : "", freespace: "",	usedspace: "" }], 
  });
  ...
  const handleAddFile = () => {
    setFormData({
      ...formData,
      files: [...formData.files, { servername: "", harddrive : "", totalspace : "", freespace: "",	usedspace: "" }],
    });
  };
  ...
  const fetchDataAuto = async (storeCode, date) => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/serverstorage-status/`, { params });
  ...
  const handleSubmit = async (e) => {
    ...
    const incompleteFiles = formData.files.map((file, index) => {
      ...
      if (!file.harddrive) missingFileFields.push('Hard Drive');
      if (!file.totalspace) missingFileFields.push('Total Space');
      if (!file.freespace) missingFileFields.push('Free Space');
      if (!file.usedspace) missingFileFields.push('Used Space');
    ...
    // If there are any missing form fields
    ...
    try {
      const checkResponse = await axios.get(`${API_BASE_URL}/api/serverstorage-status/`, {
      ...
      for (let file of formData.files) {
        const existingEntry = existingEntries.find(entry =>
          ...
          entry.harddrive === file.harddrive &&
        ...
        if (existingEntry && existingEntry.verified) {
          setSnackbar({
            open: true,
            message: `Entry for Server ${file.servername} is already verified and cannot be modified.`,
            severity: 'error',
          });
        ...
        if (existingEntry) {
          response = await axios.put(
            `${API_BASE_URL}/api/update-serverstorage-status/${existingEntry.id}/`,
            { ...existingEntry, ...file,  user: username, remarks: formData.remarks }
          );
        } else {
          response = await axios.post(`${API_BASE_URL}/api/submit-serverstorage-status/`, {
            ...dataToPost,
            files: [file],
          });
        }
    ...
    try {
      ...
      if (idsToVerify.length > 0) {
        await Promise.all(idsToVerify.map(id =>
          axios.put(`${API_BASE_URL}/api/update-serverstorage-status/${id}/`, { 
  ...
  const fetchDataForEdit = async (id) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/serverstorage-status/${id}/`);
  ...
  const handleDownloadCSV = () => {
    ...
    axios
      .get(`${API_BASE_URL}/api/serverstorage-status/download/`, {
        params,
        responseType: "json",
      })
      .then((response) => {
        ...
        link.setAttribute("download", `Server_Storage_Status_${date}.csv`);
  ...
  const convertToCSV = (jsonData) => {
    const header = ["Date",  "Store", "Store Name",  "Server Name", "Hard Drive", "Total Space (GB)", "Free Space (GB)","Used Space (GB)", "Remarks"];
  ...
  //     format: [400, 230],
  //   });
  //   const tableColumns = ["Date", "Store", "Store Name",  "Server Name", "Hard Drive", "Total Space (GB)", "Free Space (GB)","Used Space (GB)", "Remarks"];
  ...
  //   doc.save(`Server_Storage_Status_${date}.pdf`);
  // };
  ...
  const handleDownloadPDF = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const tableColumns = [
          "Date", "Store", "Store Name", "Server Name", "Hard Drive",
          "Total Space (GB)", "Free Space (GB)", "Used Space (GB)", "Remarks"
        ];
  ...
  const handleSaveClick = async () => {
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/update-serverstorage-status/${editRowId}/`,
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (confirmed) {
      try {
        const response = await axios.delete(`${API_BASE_URL}/api/delete-serverstorage-status/${id}/`, {
          data: { user: username }  
        });
        if (response.status === 200) {
  ...
  const fetchHistory = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/serverstorage-status/`, {
  ...
  const getTabsForUserGroup = () => {
    switch (userGroup) {
      case 'Admin_User':
        return (
          <>
            <Tab value={0}>Server Storage Form</Tab>
            <Tab value={1}>Server Storage Status</Tab>
            <Tab value={2}>History</Tab>
          </>
        );
      case 'End_User':
        return <Tab value={0}>Server Storage Form</Tab>
                ...
                {isEditMode ? (
                  <form onSubmit={handleSubmit}>
                    <Box sx={{ display: 'flex', gap: 2, marginBottom: 2 }}>
                      <TextField label="Date" name="date" type="date" fullWidth value={formData.date} onChange={handleChange}
                    ...
                    {formData.files.map((file, index) => (
                      <Box key={index} sx={{ display: 'flex', gap: 2, marginBottom: 2 }}>
                        ...
                        />
                        <FormControl fullWidth>
                          <InputLabel id={`harddrive-label-${index}`} sx={{
                            fontSize: '1rem',
                            backgroundColor: 'white',
                            px: 0.5,
                            transform: 'translate(14px, 14px) scale(1)',
                          ...
                          <Select
                            labelId={`harddrive-label-${index}`}
                            value={file.harddrive}
                            onChange={(e) => handleFileChange(index, "harddrive", e.target.value)}
                            sx={{
                              fontSize: '0.8rem',
                              height: '56px',
                              '& .MuiInputBase-input': {
                          ...
                          >
                            <MenuItem value="C-DRIVE (Disk)">C-DRIVE (Disk)</MenuItem>
                            <MenuItem value="D-DRIVE (Elements)">D-DRIVE (Elements)</MenuItem>
                            <MenuItem value="F-DRIVE (Application)">F-DRIVE (Application)</MenuItem>
                            <MenuItem value="G-DRIVE (Backup)">G-DRIVE (Backup)</MenuItem>
                          </Select>
                        </FormControl>
                        <TextField label="Total Space (GB)" type="number" fullWidth value={file.totalspace} onChange={(e) => handleFileChange(index, "totalspace", e.target.value)}
                          sx={{ fontSize: '0.9rem', boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)'}}
                        />
                        <TextField label="Free Space (GB)" type="number" fullWidth value={file.freespace} onChange={(e) => handleFileChange(index, "freespace", e.target.value)}
                          sx={{ fontSize: '0.9rem', boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)'}}
                        />
                        <TextField label="Used Space (GB)" type="number" fullWidth value={file.usedspace} onChange={(e) => handleFileChange(index, "usedspace", e.target.value)}
                          sx={{ fontSize: '0.9rem', boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)'}}
                        />
                    ...
                    <Box sx={{ marginBottom: 4 }}>
                      <TextField label="Remarks (Eg:Ticket No.)" fullWidth multiline rows={4} name="remarks" value={formData.remarks} onChange={handleChange} 
                  ...
                  <Box sx={{ padding: 4, boxShadow: 2, maxWidth: 1200 }}>
                    ...
                    <TableContainer>
                      ...
                      <TableHead>
                          <TableRow>
                            <TableCell colSpan={3} style={{ borderBottom: '1px solid #ccc', padding: '10px 25px', whiteSpace: 'nowrap'}}><strong>Store</strong></TableCell>
                            ...
                            submittedData.files.map((file, index) => (
                              <React.Fragment key={index}>
                                <TableRow>
                                  ...
                                  <TableCell colSpan={3} align="center">{file?.servername}</TableCell>
                                  <TableCell style={{ borderBottom: '1px solid #ccc', padding: '10px 25px', whiteSpace: 'nowrap' }}>Hard Drive</TableCell>
                                  <TableCell colSpan={3} align="center">{file?.harddrive}</TableCell>
                                </TableRow>
                                <TableRow>
                                  <TableCell style={{ borderBottom: '1px solid #ccc', padding: '10px 25px', whiteSpace: 'nowrap' }}>Total Space (GB)</TableCell>
                                  <TableCell align="center">{file?.totalspace}</TableCell>
                                  <TableCell style={{ borderBottom: '1px solid #ccc', padding: '10px 25px', whiteSpace: 'nowrap' }}>Free Space (GB)</TableCell>
                                  <TableCell align="center">{file?.freespace}</TableCell>
                                  <TableCell style={{ borderBottom: '1px solid #ccc', padding: '10px 25px', whiteSpace: 'nowrap' }}>Used Space (GB)</TableCell>
                                  <TableCell align="center">{file?.usedspace}</TableCell>
                                </TableRow>
                              </React.Fragment>
                            ))
                          ) : (
                    ...
                    <Table sx={{ borderCollapse: 'collapse', border: '2px solid #113f6c' }}>
                      <TableHead sx={{ position: 'sticky', top: 0, backgroundColor: '#f5f5f5', zIndex: 1000}}>
                        <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                          ...
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '8%' }}>
                            <TableSortLabel active={sortConfig.key === 'harddrive'} direction={sortConfig.key === 'harddrive' ? sortConfig.direction : 'asc'} onClick={() => handleSort('harddrive')}>Hard Drive</TableSortLabel>
                          </TableCell>
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '20%' }}>
                            <TableSortLabel active={sortConfig.key === 'totalspace'} direction={sortConfig.key === 'totalspace' ? sortConfig.direction : 'asc'} onClick={() => handleSort('totalspace')}>Total Space</TableSortLabel>
                          </TableCell>
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '8%' }}>
                            <TableSortLabel active={sortConfig.key === 'freespace'} direction={sortConfig.key === 'freespace' ? sortConfig.direction : 'asc'} onClick={() => handleSort('freespace')}>Free Space</TableSortLabel>
                          </TableCell>
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color: '#113f6c', fontSize: '0.9rem', width: '10%' }}>
                            <TableSortLabel active={sortConfig.key === 'usedspace'} direction={sortConfig.key === 'usedspace' ? sortConfig.direction : 'asc'} onClick={() => handleSort('usedspace')}>Used Space</TableSortLabel>
                          </TableCell>
                          ...
                          <TableRow key={item.id} hover
                            ...
                            {editRowId === item.id ? (
                              <>
                                ...
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                                  <Select  name="harddrive"  value={updatedRow.harddrive || ''}  onChange={handleInputChange}  fullWidth 
                                ...
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                                  <input type="number" name="totalspace" value={updatedRow.totalspace|| ''} onChange={handleInputChange} style={{ width: '100%', height: '40px' }} />
                                </TableCell>
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                                  <input type="number" name="freespace" value={updatedRow.freespace|| ''} onChange={handleInputChange} style={{ width: '100%', height: '40px' }} />
                                </TableCell>
                                <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                                  <input type="number" name="usedspace" value={updatedRow.usedspace|| ''} onChange={handleInputChange} style={{ width: '100%', height: '40px' }} />
                                </TableCell>
                              ...
                              <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%'}}>{item.harddrive}</TableCell>
                              <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%'}}>{item.totalspace}</TableCell>
                              <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%'}}>{item.freespace}</TableCell>
                              <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%'}}>{item.usedspace}</TableCell>
...
export default ServerStorageStatus