...
const NasBackup  = () => {
  ...
  const [failed_dates, setFailedDates] = useState([]); // Store multiple failed dates
  ...
  const siteName = formData.store ? formData.store.replace(/\s+/g, '_') : 'Unknown_Site';
  const selectedMonth = formData.reviewMonth ? formData.reviewMonth.replace(/\s+/g, '_') : 'Unknown_Month';
  ...
  const dataToSubmit = {
      ...
      verifierSignature: formData.verifierSignature || null,
      site: formData.store || "",
      fileName1: formData.fileName1, 
      fileName2: formData.fileName2, 
      dateCopied1: formData.dateCopied1 ? formData.dateCopied1.split("T")[0] : null,
      dateCopied2: formData.dateCopied2 ? formData.dateCopied2.split("T")[0] : null,
      size1: formData.size1 || "",
      size2: formData.size2 || "",
      serverName: formData.serverName || "",
      ipAddress: validateIPAddress(formData.ipAddress) ? formData.ipAddress : null,
      frequency: formData.frequency || "",
      typeOfBackup: formData.typeOfBackup || "",
      failed_dates: failed_dates || "",
      days: formData.days || "",
      fileName: generatedFileName,  // ✅ Add the generated file name here
    };
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        setIsEditMode(false);
        setSubmittedData({ ...dataToSubmit, fileName: generatedFileName }); 
      } else {
  ...
  const handleSubmit1 = async () => {
    ...
    const fileName = `${formData1.store}_POS_User_Status_${formData1.month}_${new Date().getFullYear()}_Varified.${fileExtension}`;
    const currentYear = new Date().getFullYear();
  
    const data = new FormData();
    data.append('store', formData1.store.trim());
    data.append('month', formData1.reviewMonth1);
    ...
    try {
      ...
      const checkResponse = await fetch(`${API_BASE_URL}/api/backup-detailVerified/?store=${formData1.store}`);
      const checkData = await checkResponse.json();
  
      let response;
      const dataToSubmit = new FormData();
      dataToSubmit.append("store", formData1.store.trim());
      dataToSubmit.append("file", formData1.file);
      dataToSubmit.append("reviewMonth1", formData1.reviewMonth1 || "Unknown");
  
      if (checkResponse.ok && checkData.length > 0) {
          // Record exists -> Update it (PUT)
          const existingEntry = checkData[0];
          response = await fetch(`${API_BASE_URL}/api/backup-detailVerified/${existingEntry.id}/`, {
              method: "PUT",
              body: dataToSubmit,
          });
      } else {
          // No record -> Create a new one (POST)
          response = await fetch(`${API_BASE_URL}/api/backup-detailVerified/`, {
              method: "POST",
              body: dataToSubmit,
          });
      }
      ...
      if (response.ok) {
          ...
          // ✅ Ensure we set submittedData1 correctly
          setSubmittedData1({
              ...formData1,
              fileName,
  ...
  const addFailedDate = () => {
    setFailedDates([...failed_dates, ""]); // Add an empty input
  };
  ...
  useEffect(() => {
    // Parse failed_dates from the data response
    const parsedFailedDates = JSON.parse(data.failed_dates || "[]");
    setFailedDates(parsedFailedDates); // Set the parsed failed dates
  ...
  const fetchData1 = async () => {
      if (!formData1.reviewMonth1) {  // ✅ Use formData1.reviewMonth1
          alert("Please select the review month.");
          return;
      }
  
      try {
          const params = { reviewMonth1: formData1.reviewMonth1 };  // ✅ Extract from formData1
  ...
  const handleDownloadZIP = async () => {
    ...
    // Filter files and fetch them
    const filePromises = data
      .filter((item) => item.file) // Ensure there's a file
      .map(async (item) => {
        try {
          const response = await fetch(item.file);
          if (!response.ok) throw new Error(`Failed to fetch ${item.file}`);
          const blob = await response.blob();
          const fileName = item.file.split('/').pop(); // Extract file name
  ...
  const handleDownloadPDF = () => {
    ...
    const xPosition = pageWidth - imageWidth - 10; 
    const yPosition = 10; 
    ...
    doc.setFontSize(12);
    doc.setTextColor(0); 
    doc.text('Date:', xPosition - 20, 20); 
    doc.text(formattedDate, xPosition-10, 20); 
    doc.setFontSize(12);
    doc.setTextColor(0);
    ...
    const extractMonthYear = (dateString) => {
      if (!dateString) {
        console.warn("Warning: formData.date is undefined. Falling back to current date.");
        dateString = getDefaultDate(); 
      }
      const [year, month] = dateString.split('-').map(Number);
      return { month, year };
    };
    const { month, year } = extractMonthYear(formData.date || getDefaultDate());
    console.log("Extracted Month:", month, "Year:", year);
    const daysInSelectedMonth = month && year ? getDaysInMonth(month, year) : 31; 
    const tableColumns = [
      'Sl No',
      'Server',
      'IP Address',
    ...
    legendItems.forEach((item, index) => {
      doc.text(`${item.key}`, 14, startY + index * 5); 
      doc.text(`${item.description}`, 20, startY + index * 5); 
    });
    doc.setFontSize(12);
    doc.setTextColor(0);
    doc.text('Verified By:', 14, doc.lastAutoTable.finalY + 30); 
    doc.text('Sreejith CS', 300, doc.lastAutoTable.finalY + 30);
    doc.text('(Manager - IT Operations)', 300, doc.lastAutoTable.finalY + 38);
    doc.text('Date:', 14, doc.lastAutoTable.finalY + 38); 
    doc.text(new Date().toLocaleDateString(), 30, doc.lastAutoTable.finalY + 38); 
    const siteName = data[0]?.store ? data[0].store.replace(/\s+/g, '_') : 'Unknown_Site';
    ...
    switch (userGroup) {
      case 'Admin_User':
        return (
          ...
          <Tab value={1}>NAS Backup Upload</Tab>
          <Tab value={2}>NAS Backup List</Tab>
          </>
        );
      case 'End_User':
        return( 
  ...
  <Box sx={{ marginBottom: 2 }}>
      {/* Show message if no failed dates exist */}
      {failed_dates.length === 0 ? (
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <Typography variant="body1" color="textSecondary">
            If there are any failed backups, kindly provide the corresponding dates.
          </Typography>
          <IconButton color="primary" onClick={addFailedDate}>
            <AddIcon />
          </IconButton>
        </Box>
      ) : (
        failed_dates.map((date, index) => (
          <Box key={index} sx={{ display: "flex", gap: 1, alignItems: "center", marginBottom: 1 }}>
            <TextField
              type="date"
              value={date}
              onChange={(e) => updateFailedDate(index, e.target.value)}
              InputLabelProps={{ shrink: true }}
              sx={{ fontSize: "0.75rem", boxShadow: "0px 4px 6px rgba(0, 0, 0, 0.1)" }}
            />
            {/* "-" Icon */}
            <IconButton color="error" onClick={() => removeFailedDate(index)}>
              <RemoveIcon />
            </IconButton>
                              ...
                              <TableCell>Review Month</TableCell>
                                                      ...
                                                      <TableCell align="center" sx={{ whiteSpace: 'nowrap' }}>
                                                              {submittedData?.fileName && (
                                                                <Button onClick={handleDownloadPDF} sx={{whiteSpace: 'nowrap', fontSize: '0.75rem' }}>
                                                                  {submittedData.fileName}
                                                                </Button>
                                                              )}
                                                            </TableCell>
    ...
    <Paper sx={{ overflow: 'auto', boxShadow: 3 }}>
      <TableContainer>
        <Table>
          <TableHead>
            ...
            <TableRow>
              <TableCell><strong>Review Month</strong></TableCell>
      ...
      >
        ...
        Download All
                ...
                <Sheet sx={{marginBottom: 1, marginLeft: 3, marginRight: 3 }}>
                  <TableContainer component={Paper} sx={{ marginTop: 10, marginLeft: 1, marginRight: 1 }}>
                    ...
                    <Table sx={{ borderCollapse: 'collapse', border: '2px solid #113f6c' }}>
                      <TableHead>
                        <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                          ...
                          <TableCell sx={{ border: '1px solid #113f6c', fontWeight: 'bold', padding: '8px 16px', color : '#113f6c', fontSize: '0.9rem', width: '5%' }}>Review Month</TableCell>
      ...
      sx={{ whiteSpace: "nowrap", fontSize: "0.75rem", color: "#1976d2", textTransform: "none" }}
    >
      {item.file ? item.file.split("/").pop() : "No File"}
    </Button>
