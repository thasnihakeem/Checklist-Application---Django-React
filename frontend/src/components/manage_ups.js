...
import { Grid, Paper, Button, Box , Typography, TextField, FormControl, InputLabel, Select, MenuItem } from '@mui/material'; 
...
const ManageUPS = () => {
  const [formData, setFormData] = useState({
      action:"",
      store: "",
      files: [{ ups: "", serialnumber : "", vendorname:"", brand:"", area:"" , capacity: "", warrantyexp:"", amcstartdate:"", amcenddate:""  }], 
    });
  ...
  const handleAddFile = () => {
    setFormData({
      ...formData,
      files: [...formData.files, { ups: "", serialnumber : "", vendorname:"", brand:"", area:"" , capacity: "", warrantyexp:"", amcstartdate:"", amcenddate:""  }],
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const checkResponse = await axios.get(`${API_BASE_URL}/api/ups/`, {
        ...
        for (let file of formData.files) {
          ...
          if (existingEntry) {
              response = await axios.put(
                  `${API_BASE_URL}/api/update-ups/${existingEntry.id}/`,
                  { ...existingEntry, ...file }
              );
              savedEntries.push(response.data);
          } else {
              response = await axios.post(`${API_BASE_URL}/api/submit-ups/`, {
                  ...dataToPost,
                  files: [file],
              });
              savedEntries.push(response.data);
          ...
          for (let file of formData.files) {
              ...
              if (existingEntry) {
                  response = await axios.delete(
                      `${API_BASE_URL}/api/delete-ups/${existingEntry.id}/`
                  );
              }
          }
        }
    ...
    const fetchDataForEdit = async (id) => {
      ...
      try {
        const response = await axios.get(`${API_BASE_URL}/api/ups/${id}/`);
    ...
    const fetchData = async () => {
      ...
      try {
          ...
          const response = await axios.get(`${API_BASE_URL}/api/ups/`, { params });
    ...
    const handleDownloadCSV = () => {
      ...
      axios
        .get(`${API_BASE_URL}/api/ups/download/`, {
          params,
          responseType: "json",
        })
        .then((response) => {
          ...
          link.setAttribute("download", `UPS_Details_${store}.csv`);
    ...
    const convertToCSV = (jsonData) => {
      const header = ["Store", "UPS Name", "Serial Number", "Vendor Name", "Brand", "Area", "Capacity", "Warranty Date", "AMC Start Date", "AMC End Date"];
    ...
    const handleDownloadPDF = () => {
      ...
      const tableColumns = ["Store", "UPS Name", "Serial Number", "Vendor Name", "Brand", "Area", "Capacity", "Warranty Date", "AMC Start Date", "AMC End Date"];
      ...
      doc.save(`UPS_details_${store}.pdf`);
    };
    ...
    const handleSaveClick = async () => {
      try {
        const response = await axios.put(
          `${API_BASE_URL}/api/update-ups/${editRowId}/`,
          updatedRow
        );
    ...
    const handleDeleteClick = async (id) => {
      ...
      if (confirmed) {
        try {
          const response = await axios.delete(`${API_BASE_URL}/api/delete-ups/${id}/`);
          if (response.status === 200) {
  ...
  return (
    <div style={{ display: 'flex' }}>
      <Tabs defaultValue={0}>
        <TabsList>
          <Tab value={0}>Manage UPS</Tab>
          <Tab value={1}>UPS Details</Tab>
        </TabsList>
        <TabPanel value={0}>
                      ...
                      <TableRow key={item.id} hover
                        ...
                        {editRowId === item.id ? (
                          <>
                            ...
                            <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                              <input type="text" name="amcstartdate" value={updatedRow.amcstartdate|| ''} onChange={handleInputChange} style={{ width: '100%', height: '40px' }} />
                            </TableCell>
                            <TableCell sx={{ border: '1px solid #113f6c', padding: '9px 16px', fontSize: '0.85rem', width: '10%' }}>
                              <input type="text" name="amcenddate" value={updatedRow.amcenddate|| ''} onChange={handleInputChange} style={{ width: '100%', height: '40px' }} />
                            </TableCell>
