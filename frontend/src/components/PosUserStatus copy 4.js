...
import ImageViewer from "./ImageViewer"; // if using image modal
...
const PosUserStatus = () => {
  ...
  const handleFileChangeByCategory = (e, category) => {
  ...
  const handleCancelFileByCategory = (category, index) => {
    setFormData1(prev => {
  ...
  const handleSubmit1 = async () => {
    ...
    const uploadCategories = ["signoffFiles", "totalUserFiles", "activeUserFiles", "inactiveUserFiles"];
    let hasFiles = false;
  
    let fileName = "";

    uploadCategories.forEach(category => {
      if (formData1[category] && formData1[category].length > 0) {
        hasFiles = true;
        formData1[category].forEach((file, index) => {
          data.append(category, file);
          if (!fileName) {
            fileName = `${formData1.store}_POS_User_Status_${formData1.month}_${currentYear}_Verified_${category}_${file.name}`;
          }
        });
      }
    });    
  ...
  const groupedFiles = normalizedFiles.reduce((acc, file) => {
    const match = file.file_name.match(/^(\d{4})_/); // Extracts 4-digit store code
    const storeCode = match ? match[1] : 'Unknown';
  
    if (!acc[storeCode]) {
      acc[storeCode] = [];
    }
  
    acc[storeCode].push(file);
    return acc;
  }, {});
  ...
  const handleViewAll = (month, storeCode) => {
    const filesForStore = groupedFiles[storeCode] || [];
  
    const filesGroupedByMonth = filesForStore.reduce((acc, file) => {
      const fileMonth = file.file_name.split('_')[4];
      if (!acc[fileMonth]) acc[fileMonth] = [];
      acc[fileMonth].push(file);
      return acc;
    }, {});
  
    const filesForMonth = filesGroupedByMonth[month];
  
    if (filesForMonth && filesForMonth.length > 0) {
      setCurrentFileList(filesForMonth);
      setCurrentFileIndex(0);
      openFileInViewer(filesForMonth[0]);
    } else {
      setSnackbarMessage("No files found for the selected month and store.");
  ...
  const openFileInViewer = (file) => {
    ...
    if (fileExtension === "xlsx") {
      ...
      setDocxFileUrl(fileUrl); 
      setIsDocxViewerOpen(true);
    } else if (["pdf", "png","png", "jpg", "jpeg"].includes(fileExtension)) {
      //window.open(fileUrl, "_blank");
      setImageUrl(fileUrl, '_blank');
      setIsImageViewerOpen(true);
    } else {
      setSnackbarMessage("Unsupported file type.");
      setSnackbarSeverity("error");
  ...
  const handleNext = () => {
    ...
    if (nextIndex < currentFileList.length) {
      setCurrentFileIndex(nextIndex);
      openFileInViewer(currentFileList[nextIndex]);
    }
  };
  ...
  const handlePrev = () => {
    ...
    if (prevIndex >= 0) {
      ...
      openFileInViewer(currentFileList[prevIndex]);
    }
  };

  const handleDeleteMonth = async (monthFiles, storeCode) => {
    const confirmDelete = window.confirm(
      "Are you sure you want to delete all files for this month?"
    );
    if (!confirmDelete) return;
  
    // Build a set of file_urls to delete
    const urlsToDelete = new Set(monthFiles.map(f => f.file_url));
  
    // Sequentially delete one by one, and remove from state as we go
    for (let file of monthFiles) {
      try {
        const res = await fetch(`${API_BASE_URL}/api/delete-posuserstatus/`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ file_url: file.file_url }),
        });
        const data = await res.json();
  
        if (res.ok && data.message === "File deleted successfully") {
          // remove this single file from the flat array
          setFiles(prev => prev.filter(f => f.file_url !== file.file_url));
        } else {
          console.error("Failed to delete", file.file_name, data);
        }
      } catch (err) {
    ...
    // All done
    setSnackbarMessage("Finished deleting files for this month.");
    setSnackbarSeverity("success");
    setSnackbarOpen(true);
  };
  
  const handleDownloadMonth = async (monthFiles, storeCode, month, year) => {
    if (monthFiles.length === 0) {
      setSnackbarMessage("No files available to download for the selected month.");
      setSnackbarSeverity("warning");
      setSnackbarOpen(true);
      return;
    }
    ...
    if (!month || !year) {
      setSnackbarMessage("Month and year are required for downloading.");
      setSnackbarSeverity("error");
      setSnackbarOpen(true);
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const link = document.createElement("a");
        link.href = URL.createObjectURL(blob);
        link.download = `POS_User_Status_${storeCode}_${month}_${year}.zip`;
        link.click();
        ...
        setSnackbarMessage("Error fetching ZIP file.");
        setSnackbarSeverity("error");
        setSnackbarOpen(true);
      }
    } catch (error) {
      console.error("Error downloading ZIP:", error);
      setSnackbarMessage("Error downloading ZIP.");
      setSnackbarSeverity("error");
      setSnackbarOpen(true);
    }
  };
      ...
      case 'Admin_User':
        return (
          <>
            ...
            <TabPanel value={1}>
              <Box sx={{ padding: 4, maxWidth: 650, margin: '0 auto', minHeight: '1000px' , marginTop: '-20px' }}>
                <Paper sx={{ padding: 4, borderRadius: 2, boxShadow: 3, border: '2px solid #113f6c' }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                    <Typography variant="h6" sx={{ fontWeight: 'bold', fontSize: '1.2rem' }}>
                  ...
                  {isEditMode1 ? (
                    <form1 onSubmit={handleSubmit1}>
                      ...
                      <Typography variant="subtitle2" sx={{ color: 'red', mb: 2 }}>
                        Ensure timestamp is visible in the files uploading
                      </Typography>

                      {[
                        { label: "Signoff Form", name: "signoffFiles" },
                        { label: "Total User List", name: "totalUserFiles" },
                        { label: "Active User List", name: "activeUserFiles" },
                        { label: "Inactive User List", name: "inactiveUserFiles" },
                      ].map(({ label, name }) => (
                        <Box key={name} sx={{ marginBottom: 2, boxShadow: 2, borderRadius: 2, padding: 2, border: '1px solid #113f6c', backgroundColor: '#f7f9fc' }}>
                          <Typography variant="subtitle1" sx={{ marginBottom: 1, fontWeight: 'bold', color: '#113f6c' }}>{label}</Typography>
                          <Box sx={{ display: 'flex', alignItems: 'center', padding: 2, border: '2px dashed #113f6c', borderRadius: 1, backgroundColor: '#ffffff' }}>
                            <input
                              type="file"
                              ...
                              style={{ display: 'none' }}
                              id={`file-upload-${name}`}
                              onChange={(e) => handleFileChangeByCategory(e, name)}
                            />
                            <label htmlFor={`file-upload-${name}`} style={{ width: '100%', cursor: 'pointer' }}>
                          ...
                          {formData1[name] && formData1[name].length > 0 && formData1[name].map((file, index) => (
                            <Box key={index} sx={{ mt: 1, display: 'flex', justifyContent: 'space-between' }}>
                              <Typography variant="body2" sx={{ fontWeight: 'bold', color: '#113f6c' }}>{file.name}</Typography>
                              <Button variant="outlined" color="error" size="small" onClick={() => handleCancelFileByCategory(name, index)}>
                                Cancel
                              </Button>
                            </Box>
                          ))}
                    ...
                    <Box>
                      <Paper sx={{ overflow: 'auto', boxShadow: 3, padding: 2 }}>
                        <TableContainer>
                          <Table>
                            <TableBody>
                              <TableRow>
                                <TableCell><strong>Store</strong></TableCell>
                                <TableCell align="left">{submittedData1?.store || ''}</TableCell>
                              </TableRow>
                              <TableRow>
                                <TableCell><strong>Store Name</strong></TableCell>
                                <TableCell align="left">{submittedData1?.storename || ''}</TableCell>
                              </TableRow>
                              <TableRow>
                                <TableCell><strong>Review of Month</strong></TableCell>
                                <TableCell align="left">{submittedData1?.month || ''}</TableCell>
                              </TableRow>
                            </TableBody>
                          </Table>
                        </TableContainer>
                        ...
                        <Box sx={{ mt: 3 }}>
                          <Typography variant="h6" sx={{ mb: 2, fontWeight: 'bold', color: '#113f6c' }}>
                            Uploaded Files
                          </Typography>

                          {[
                            { label: "Signoff Form", key: "signoffFiles" },
                            { label: "Total User List", key: "totalUserFiles" },
                            { label: "Active User List", key: "activeUserFiles" },
                            { label: "Inactive User List", key: "inactiveUserFiles" },
                          ].map(({ label, key }) => (
                            <Box key={key} sx={{ mb: 2, p: 2, borderRadius: 1, border: '1px solid #ccc', backgroundColor: '#f9f9f9' }}>
                              <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: '#113f6c', mb: 1 }}>
                                {label}
                              </Typography>

                              {submittedData1?.[key] && submittedData1[key].length > 0 ? (
                                <ol style={{ paddingLeft: '20px' }}>
                                  {submittedData1[key].map((url, index) => (
                                    <li key={index}>
                                      <a
                                        href={`${API_BASE_URL}/${url}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        style={{ fontSize: '0.8rem', textDecoration: 'underline', color: '#1976d2' }}
                                      >
                                        View File {index + 1}
                                      </a>
                                    </li>
            ...
            <TabPanel value={2}>
              <div>
                ...
                <Box sx={{ marginTop: '70px', marginLeft: '30px', maxHeight: '610px', maxWidth: '1410px', overflowY: 'auto', padding: '16px', border: '1px solid #ddd', borderRadius: '8px', boxShadow: 3 }}>
                  <Stack spacing={2}>
                    {Object.entries(groupedFiles).map(([storeCode, files]) => {
                      // Group files by month for each store
                      const filesGroupedByMonth = files.reduce((acc, file) => {
                        const month = file.file_name.split('_')[4]; // Assuming month is the third part of the filename
                        if (!acc[month]) {
                          acc[month] = [];
                        }
                        acc[month].push(file);
                      ...
                      return (
                        <Accordion key={storeCode} sx={{ boxShadow: 3 }}>
                          <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls={`panel-${storeCode}-content`}>
                            <Typography fontWeight="bold">{storeCode}</Typography>
                          </AccordionSummary>
                          <AccordionDetails>
                            <Stack spacing={1}>
                              {Object.entries(filesGroupedByMonth).map(([month, monthFiles]) => (
                                <Accordion key={month} sx={{ boxShadow: 3 }}>
                                  <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls={`panel-${storeCode}-${month}-content`}>
                                    <Typography fontWeight="bold" >{month}</Typography>
                                    <Box sx={{ display: 'flex', gap: '10px' }}>
                                      <Button
                                          variant="outlined"
                                          size="small"
                                          onClick={() => handleViewAll(month, storeCode)}
                                          sx={{
                                            color: "#113f6c",
                                            marginLeft: "900px",
                                            fontSize: "0.7rem",
                                      ...
                                      <ImageViewer
                                        ...
                                        isLastFile={currentFileIndex === currentFileList.length - 1}
                                      />

                                      <Button
                                        variant="contained"
                                        size="small"
                                        onClick={() => handleDownloadMonth(monthFiles, storeCode, month, year)}
                                        sx={{
                                      ...
                                      {/* Delete the Month's Files */}
                                      <Button
                                        variant="outlined"
                                        size="small"
                                        onClick={() => handleDeleteMonth(monthFiles, storeCode, month)}
                                        sx={{
                                          color: '#f44336',
                                          fontSize: '0.7rem',
                                          padding: '6px 12px',
                                        ...
                                        {monthFiles.map((file) => (
                                          ...
                                          >
                                            <CardContent sx={{ flexGrow: 1 }}>
                                              <Typography variant="body2" noWrap fontWeight="bold" sx={{ textAlign: 'left', fontSize: '0.875rem' }}>
                                                {file.file_name}
                                              </Typography>
                                            </CardContent>
                                            <Box sx={{ display: 'flex', gap: '10px' }}>
